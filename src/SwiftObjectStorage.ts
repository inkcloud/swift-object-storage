 ...
 */
export default class SwiftObjectStorage {
  tokenCache: Cache;
   ...
   */
	...
	async getObjectWriteToStream(writeStream: WriteStream, container: string, filename: string, options: GetObjectAsStreamOptions = {}) {
   ...
   * @param options
   */
	async getRawCreateStream(container, filename: string, options: CreateObjectFromStreamOptions = {}): Promise<Request> {
		const tokenData = await this.getTokenData();
		return Promise.resolve(request.put({
			url: `${tokenData.objectPath}/${container}/${filename}`,
			timeout: 1000 * 60 * 10,
			headers: {
				...options.headers,
	...
	createObjectFromStream(sourceStream: ReadStream, container: string, filename: string, options: CreateObjectFromStreamOptions = {}) {
		return new Promise(async (resolve, reject) => {
			const putStream = await this.getRawCreateStream(container, filename, options);
